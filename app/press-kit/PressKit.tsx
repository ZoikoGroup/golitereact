...
export default function PressKit() {
  ...
  return (
    <div className="bg-[#f5f6f7] text-black">
      {/* 🔵 HERO */}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden h-fit">
            ...
            <div className="px-5 py-3 border-t border-gray-100 bg-gray-50">
              <p className="text-xs text-gray-400">Fact Sheet — March 2026</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        <div className="bg-white rounded-xl p-6 shadow-sm grid md:grid-cols-3 gap-6 text-sm">
          <div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        ...
        <div className="space-y-6">
          {leaders.map((leader, i) => (
            <div
              key={i}
              className="bg-white rounded-xl shadow-sm p-6 flex gap-6 items-start"
            >
              {/* Headshot */}
              <div className="relative w-56 h-64 flex-shrink-0 rounded-lg overflow-hidden bg-gray-200">
                <Image
                  src={leader.img}
                  alt={leader.name}
                  fill
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mb-3">{leader.companies}</p>
                <blockquote className="border-l-4 border-green-400 pl-4 text-sm text-gray-600 leading-relaxed">
                  {leader.bio}
                </blockquote>
              </div>
            </div>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        ...
        <div className="space-y-3">
          {filteredNews.map((item, i) => (
            <div
              key={i}
              className={`bg-white rounded-xl shadow-sm p-5 flex gap-5 items-start border-l-4 ${
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {visualAssets.map((asset, i) => (
            <div
              key={i}
              className={`rounded-xl overflow-hidden shadow-sm cursor-pointer group relative ${asset.bg}`}
            >
              {/* Image area */}
              <div className="relative h-28 flex items-center justify-center">
                <span className="text-4xl">{asset.emoji}</span>
                {/* Overlay on hover */}
                <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity rounded-t-xl" />
              </div>

              {/* Info */}
              <div className={`p-3 ${asset.dark ? "text-white" : "text-gray-800"}`}>
                <p className="text-xs font-semibold">{asset.title}</p>
                <p className={`text-xs mt-0.5 ${asset.dark ? "text-gray-300" : "text-gray-500"}`}>
                  {asset.desc}
                </p>
              </div>
            </div>
